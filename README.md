...
Unlike standard API wrappers, Synapse solves the heavy-compute problem of modern LLMs and vision models by splitting the workload: a **Raspberry Pi 3** acts as the sensory edge device (eyes and ears) written in bare-metal C++, while a powerful **Local PC** acts as the brain (inference server) fully leveraging **NVIDIA CUDA** for parallel GPU computing.

<img src=" https://cdn.pixabay.com/photo/2024/04/08/19/56/neural-network-8684318_1280.jpg">

## 🤖 Key Capabilities

* **Real-Time Edge Perception:** RPi 3 directly captures video (V2 Camera) and audio, applies raw OpenCV MPEG compression, and streams it over LAN with near-zero latency.
* **CUDA-Accelerated Conversational AI:** Heavy lifting is done entirely on the GPU. Powered by OpenAI's **Whisper** (distil-large-v3) for real-time transcription and **Ollama (Qwen 2.5 - 3B)** for insanely fast, context-aware, and chatty responses utilizing full Tensor Core acceleration.
...
* **Tech Stack:** Pure C++, OpenCV, SDL2, Miniaudio.
...
* **Tech Stack:** Python, ZeroMQ, cuDNN, OpenAI Whisper (distil-large-v3), Ollama (Qwen 2.5 - 3B), Kokoro TTS, InsightFace, ChromaDB.
* **Function:** A multi-threaded Python backend that ingests the socket stream asynchronously. It handles wake-word detection, pushes audio and vision tasks to the GPU for inference, generates RAG-backed responses, synthesizes speech, and streams the audio back to the Edge.

### 🔌 Network & Shell Orchestration
The magic tying these two nodes together isn't a bloated web framework; it's raw networking and scripting:
...
* **Bash/Shell Scripting:** The entire startup sequence, network binding, environment variable management, and process daemonization on both the RPi and the Linux server are heavily automated using custom shell scripts (`.sh`).
...
*I am documenting this entire engineering journey as a multi-part "Web Series" style Devlog on Medium. (Links dropping soon).*

* **Episode 1: The Edge Struggle.** Why I chose pure C++ for the Pi, fighting with SDL2/Miniaudio, and writing custom OpenCV MPEG compression to kill latency.
* **Episode 2: Networking Hell.** Tying the Pi and PC together. Why standard HTTP failed and how ZeroMQ + Shell Scripting saved the day.
